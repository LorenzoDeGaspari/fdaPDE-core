...
int main(){
    ...
    testData test_ref0("Refinement 0", 36, "data/quad_kx_ref0.mtx", "data/quad_ky_ref0.mtx",
                       ...
                       "data/quad_cpy_ref0.mtx", "data/quad_dr_geopde_sol_36.mtx");
    ...
    testData test_ref1("Refinement 1", 64, "data/quad_kx_ref1.mtx", "data/quad_ky_ref1.mtx",
                       ...
                       "data/quad_cpy_ref1.mtx", "data/quad_dr_geopde_sol_64.mtx");
    ...
    testData test_ref2("Refinement 2", 144, "data/quad_kx_ref2.mtx", "data/quad_ky_ref2.mtx",
                       ...
                       "data/quad_cpy_ref2.mtx", "data/quad_dr_geopde_sol_144.mtx");
    ...
    testData test_ref3("Refinement 3", 324, "data/quad_kx_ref3.mtx", "data/quad_ky_ref3.mtx",
                       ...
                       "data/quad_cpy_ref3.mtx", "data/quad_dr_geopde_sol_324.mtx");
    ...
    post_processing(all_tests, "data/quad_dr_geopde_times.mtx");
    return 0;
}
...
void solve_problem(testData & test){
    ...
    // create a reaction operator
    fdapde::core::Reaction <fdapde::core::IGA, double> M(1.);
    std::cout << "DONE!\n";
    std::cout << "Creating forcing term object...\n";
    // wrap forcing term in a scalar field
    auto f = [] (const SVector<2> & x) -> double { return ((1+(9*M_PI*M_PI)*x[0])*std::exp(1-x[0]) - (9*M_PI*M_PI +1)*x[0])*
                                                            std::sin(3*M_PI*x[1]);};
    ...
    // create a pde object
    fdapde::core::PDE<decltype(mesh),decltype(K + M), decltype(ff), fdapde::core::IGA, fdapde::core::iga_order<2>> pde_r0(mesh,K + M,ff);
    std::cout << "DONE!\n";
    ...
    auto uex = [] (const SVector<2> & x) -> double {return -x[0]*(1-std::exp(1-x[0]))*std::sin(3*M_PI*x[1]);};
    auto uex_dx = [] (const SVector<2> & x) -> double {return ((1-x[0])*std::exp(1-x[0]) - 1)*std::sin(3*M_PI*x[1]);};
    auto uex_dy = [] (const SVector<2> & x) -> double {return -3*M_PI*x[0]*(1-std::exp(1-x[0]))*std::cos(3*M_PI*x[1]);};
